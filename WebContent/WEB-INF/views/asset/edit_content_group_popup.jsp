...
<script language="JavaScript">
	$(document).ready(function() {
		...
		$('#contentGroup').validate( {
			...
			submitHandler: function () {
				...
				try{
					$.ajax({
						type: "POST",
						url: "${ctxRoot}/asset/editContentGroup.do",
						data: $('form#contentGroup').serialize(),
						success: function(data, textStatus, jqXHR){
							if (data == 'Success') {
								openInform('Success', data, function() {
									closeModalWindow('01');
									viewDetail('${contentGroup.contentGroupId}');
								});
							} else {
		...
		if ('${contentGroup.audience}' == '') {
			$("#audience").select2({placeholder: '${selectMsg}'});
		} else {
			$("#audience").val("${contentGroup.audience}".split("|")).select2({placeholder: '${selectMsg}'});
		}
		
		$("#countryOfOrigin").select2();
		$("#genre").val("${contentGroup.genre}".split(",")).select2({
			placeholder: '<spring:message code="serviceAsset.contentGroup.genre.guide"/>'
            , maximumSelectionSize: 5
        });
		$("#showType").val("${contentGroup.showType}".split("|")).select2({placeholder: '${selectMsg}'});
	});
</script>
</head>
<body>
	...
	<form:form modelAttribute="contentGroup" method="POST"
		...
		<div class="panel-heading">
			<span class="panel-title"><i class="fa fa-ellipsis-v"></i> &nbsp; <spring:message code="serviceAsset.contentGroup.edit"/></span>
		</div>
		<div class="panel-body">
					...
					<div class="input-group">
						<div class="select2-success">
							<form:select path="seriesId" cssClass="form-control" multiple="multiple">
								<form:option value="${contentGroup.seriesId}">${contentGroup.seriesName}</form:option>
							</form:select>
						</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<form:select path="episodeType" cssClass="form-control">
						<option value="">${selectMsg}</option>
						<ccode:options groupCode="${const:get('COMMONCODE', 'CG_EPISODE_TYPE')}" selected="${contentGroup.episodeType}"/>
					</form:select>
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<form:select path="rating" cssClass="form-control">
						<option value="">${selectMsg}</option>
						<ccode:options groupCode="${const:get('COMMONCODE', 'CG_RATING')}" selected="${contentGroup.rating}"/>
					</form:select>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-9">
					<div class='col-md-4' style="padding-left:0;padding-right:0">
		                <div class='input-group'>
		                	...
		                	<input type="text" id="displayRunTimeHour" name="displayRunTimeHour" value="${displayRunTimeHour}" class="form-control" placeholder="<spring:message code='serviceAsset.contentGroup.displayRunTime.hour'/>"/>
		                </div>
			        </div>
					<div class='col-md-4' style="padding-left:0;padding-right:0">
		                <div class='input-group'>
		                	<input type="text" id="displayRunTimeMin" name="displayRunTimeMin" value="${displayRunTimeMin}" class="form-control" placeholder="<spring:message code='serviceAsset.contentGroup.displayRunTime.minute'/>" />
		                </div>
			        </div>
					<div class='col-md-4' style="padding-left:0;padding-right:0">
		                <div class='input-group'>
		                	<input type="text" id="displayRunTimeSec" name="displayRunTimeSec" value="${displayRunTimeSec}" class="form-control" placeholder="<spring:message code='serviceAsset.contentGroup.displayRunTime.sec'/>" />
		                </div>
			        </div>
				</div>
			</div>
		    ...
		    <div class="form-group">
				...
				<div class="col-sm-9">
					<form:select path="countryOfOrigin" cssClass="form-control">
						<ccode:options groupCode="${const:get('COMMONCODE', 'CG_COUNTRY_OF_ORIGIN')}" selected="${contentGroup.countryOfOrigin}"/>
					</form:select>
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					${contentGroup.thumbnailSubsetName}
				</div>
			</div>
