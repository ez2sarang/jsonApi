	...
	$(document).ready(function() {
		$('#commonCode').validate( {
			...
			submitHandler: function () {
				$('#fullCode').val("${commonCode.groupCode}" + $('#code').val());
				...
				try{
					$.ajax({
						type: "POST",
						url: "${ctxRoot}/system/addCommonCode.do",
		...
		<div class="panel-heading">
			<c:choose>
				<c:when test="${commonCode.groupCode == const:get('COMMONCODE', 'ROOT_GROUP_CODE')}">
					<span class="panel-title"><i class="fa fa-ellipsis-v"></i> &nbsp; <spring:message code="code.title.codeGroupReg"/></span>
				</c:when>
				<c:otherwise>
					<span class="panel-title"><i class="fa fa-ellipsis-v"></i> &nbsp; <spring:message code="code.title.codeReg"/></span>
				</c:otherwise>
			</c:choose>
		</div>
			...
			<c:choose>
				<c:when test="${commonCode.groupCode == const:get('COMMONCODE', 'ROOT_GROUP_CODE')}">
					<div class="form-group">
						<label for="code" class="col-sm-3 control-label"><spring:message code="code.groupCodeSix"/></label>
						<div class="col-sm-9">
							<form:input path="code" cssClass="form-control" placeholder="ex)DP0001,PD0001.." />
