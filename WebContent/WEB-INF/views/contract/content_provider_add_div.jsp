...
// 		$("#idType").trigger("change");
	...
	$(document).ready(function(){
		...
		$("#categoryId").select2({
			placeholder: "<spring:message code="contract.cp.category.select"/>"
		});
...
<!-- / Javascript -->
	...
	<%-- 							<form:input class="form-control" path="priceUpperRange" /><span class="input-group-addon">%</span> --%>
				...
				<div class="form-group">
					<label for="categoryId" class="col-sm-3 control-label"><spring:message code="offer.display"/></label>
					<div class="col-sm-9">
						<div class='input-group'>
							<div class="select2-primary">
								<select id="categoryId" name="categoryId" tabindex="-1" class="form-control" multiple="multiple" disabled="true">
									<c:if test="${categoryId != null }">
										<option value="<c:out value="${categoryId}"/>" selected><c:out value="${categoryUri}"/></option>
									</c:if>
								</select>
							</div>
							...
							<span class="input-group-btn">
								<button class="btn" type="button" id="categoryClearBtn" ><spring:message code="button.clear" /></button>
							</span>
						</div>
					</div>
				</div>
